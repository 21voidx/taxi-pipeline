...
def build_table_resource(
    *,
    ...
    satu-satunya cara set timePartitioning + clustering secara programatik dari Airflow.
        ...
        labels          : dict[str, str] resource-level labels untuk BQ billing.
...
def build_bq_merge_query(
    *,
    ...
    job_labels : dict[str, str] | None
        ...
        sebagai cost per label — sangat berguna untuk analisis biaya per tabel,
        per DAG, atau per environment.
        Key/value: lowercase, maks 63 karakter, hanya [a-z0-9_-].
    ...
    Catatan:
      ...
      ini adalah level yang benar untuk BQ job labels.
    """
...
{when_matched_clause}
    ...
    # Job-level labels ditempatkan di top-level configuration (bukan di dalam "query").
    # Ini yang diindeks oleh BQ billing export dan INFORMATION_SCHEMA.JOBS_BY_PROJECT.labels.
