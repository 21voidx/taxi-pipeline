...
SCHEMA_FIELDS = [
    ...
    # ── pipeline metadata ──────────────────────────────────────────────────────
    # TIDAK masuk TABLE_COLUMNS — bukan kolom dari Postgres.
    # Diisi sebagai literal di Trino SELECT, bukan di-CAST dari source.
...
# ── Metadata literal columns ───────────────────────────────────────────────────
# Dipisah dari _TRINO_COLUMNS karena:
#   1. Tidak ada di TABLE_COLUMNS  → tidak bisa di-loop dari _COLUMNS
#   2. Tidak di-CAST dari source   → diisi sebagai SQL literal langsung
#   3. Harus ditambahkan KONSISTEN di TIGA tempat dalam _trino_insert_sql():
#        a. INSERT target list   → nama plain  (_METADATA_NAMES)
#        b. SELECT source exprs  → literal SQL (_METADATA_EXPRS)
#        c. SELECT final (CTE)   → nama plain  (_METADATA_NAMES)
#      Kalau hanya ditambah di _TRINO_COLUMNS saja (SELECT source), INSERT target
#      dan SELECT final dari CTE tidak akan punya kolom ini → Trino error column mismatch.
#   4. Juga harus ada di _bq_merge_query() → ins_cols + ins_vals + set_clause
_METADATA_NAMES: list[str] = ["_ingested_at", "_source_system"]
_METADATA_EXPRS: list[str] = [
    "CURRENT_TIMESTAMP        AS _ingested_at",    # waktu pipeline jalan (UTC)
    "'ride_ops_pg'            AS _source_system",   # literal identifier source system
]
...
def _trino_insert_sql() -> str:
    ...
    # _METADATA_NAMES ditambah di sini agar kolom _ingested_at & _source_system
    # masuk ke daftar target INSERT — tanpa ini Trino akan error "column count mismatch".
    insert_cols  = ",\n        ".join(_COLUMNS + _METADATA_NAMES)
    ...
    # _METADATA_EXPRS ditambah di sini sebagai SQL literal — BUKAN di-CAST dari source.
    trino_exprs  = ",\n            ".join(_TRINO_COLUMNS + _METADATA_EXPRS)

    # SELECT final dari CTE: cukup nama kolom plain (tanpa prefix src.)
    # _METADATA_NAMES ditambah agar _ingested_at & _source_system ikut di-SELECT dari CTE.
    final_cols   = ", ".join(_COLUMNS + _METADATA_NAMES)
...
def _bq_merge_query() -> dict:
    ...
    # _METADATA_NAMES ikut di UPDATE SET — refresh _ingested_at & _source_system
    # setiap kali baris di-UPSERT agar lineage pipeline selalu akurat.
    all_non_key  = non_key_cols + _METADATA_NAMES
    set_clause   = ",\n            ".join(
        f"T.{c} = S.{c}" for c in all_non_key
    )
    # _METADATA_NAMES ikut di INSERT cols/vals agar baris baru punya metadata lengkap.
    all_cols     = _COLUMNS + _METADATA_NAMES
