...
DAG_ID = "ingestion_hallolaundry_self_service"
LIST_URL = "https://api-docs.hallolaundry.com/api/v2/iot/transactions"
PER_PAGE = 100


@dag(
    dag_id=DAG_ID,
    schedule="0 5 * * 0",  # Sunday 05:00 WIB
    ...
    tags=["haghi-laundry", "ingestion", "self-service"],
)
def ingestion_hallolaundry_self_service():
    ...
    def extract_to_gcs() -> dict:
        ...
        LOG.info(
            "Starting self-service extraction mode=%s start_date=%s end_date=%s "
        ...
        try:
            ...
            with new_temp_ndjson("hallolaundry_self_service_") as tmp:
                temp_path = tmp.name
                for range_start, range_end in generate_date_ranges(start_date, end_date):
                    LOG.info("Starting self-service API range %s..%s", range_start, range_end)
                    for page in range(1, MAX_PAGES_PER_RANGE + 1):
                        LOG.info(
                            "Fetching self-service range=%s..%s page=%s per_page=%s",
                        ...
                        if not isinstance(records, list):
                            raise AirflowException("Self-service API 'record' must be a list.")
                        if not records:
                            LOG.info(
                                "Self-service range=%s..%s page=%s returned 0 rows. total_rows=%s",
                        ...
                        for row in records:
                            ...
                            row_count += 1
                        LOG.info(
                            "Completed self-service page range=%s..%s page=%s page_rows=%s total_rows=%s",
                            range_start, range_end, page, len(records), row_count,
                        )
                        if len(records) < PER_PAGE:
                            LOG.info(
                                "Completed self-service API range %s..%s at page=%s (last page rows=%s).",
                                range_start, range_end, page, len(records),
                            )
                            break
                        pause(PAGE_DELAY_SECONDS)
                    else:
                        raise AirflowException("Exceeded MAX_PAGES_PER_RANGE for self-service API.")
            LOG.info(
                "Self-service extraction completed total_rows=%s. Starting GCS upload.",
                row_count,
            )
            meta = upload_ndjson(
                temp_path=temp_path, entity="transaction_self_service",
                raw_table="raw_transaction_self_service", row_count=row_count,
                run_id=run_id, ingestion_date=ingestion_date, mode=mode,
                start_date=start_date, end_date=end_date,
            )
            LOG.info(
                "Self-service extraction and GCS upload completed total_rows=%s",
                row_count,
            )
            return meta
        finally:
    ...
    stg_header = make_bigquery_sql_task(
        "stg_self_service_transactions", "staging/05_stg_self_service_transactions.sql"
    )
    stg_lines = make_bigquery_sql_task(
        "stg_self_service_lines", "staging/10_stg_self_service_lines.sql"
    )
    loaded >> [stg_header, stg_lines]


ingestion_hallolaundry_self_service()